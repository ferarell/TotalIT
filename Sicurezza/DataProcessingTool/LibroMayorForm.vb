...
Public Class LibroMayorForm
    ...
    Private Sub LibroDiarioForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        dtPrint = dsMain.Tables("LibroMayor")
        Try
            LoadCompany()
    ...
    Private Sub DataProcess()
        ...
        For Each row As DataRow In dtQuery.Rows
            ...
            If Not IsDBNull(row("currency_id")) Then
                sCurrency = "USD"
            End If
            dtPrint.Rows.Add(row("date"), row("move"), row("glosa"), row("t8_code"), row("ref"), row("account_code"), row("account_name"), sCurrency, row("debit"), row("credit"), IIf(row("debit") > 0, row("amount_currency"), 0), IIf(row("credit") > 0, row("amount_currency"), 0), row("account_code"))
        Next
