Public Class InputSubject

    Private _IdMessageSubject As Integer
    Private _IdMessageConfiguration As Integer
    Private _SubjectIdentifier As String
    Private _UserCreate As String
    Private _DateCreate As DateTime
    Private _UserUpdate As String
    Private _DateUpdate As DateTime
    Public Sub New()
    End Sub

    Public Sub New(IdMessageSubject As Integer, IdMessageConfiguration As Integer, SubjectIdentifier As String, UserCreate As String, DateCreate As Date, UserUpdate As String, DateUpdate As Date)
        _IdMessageSubject = IdMessageSubject
        _IdMessageConfiguration = IdMessageConfiguration
        _SubjectIdentifier = SubjectIdentifier
        _UserCreate = UserCreate
        _DateCreate = DateCreate
        _UserUpdate = UserUpdate
        _DateUpdate = DateUpdate
    End Sub

    Public Property IdMessageSubject As Integer
        Get
            Return _IdMessageSubject
        End Get
        Set(value As Integer)
            _IdMessageSubject = value
        End Set
    End Property

    Public Property IdMessageConfiguration As Integer
        Get
            Return _IdMessageConfiguration
        End Get
        Set(value As Integer)
            _IdMessageConfiguration = value
        End Set
    End Property

    Public Property SubjectIdentifier As String
        Get
            Return _SubjectIdentifier
        End Get
        Set(value As String)
            _SubjectIdentifier = value
        End Set
    End Property

    Public Property UserCreate As String
        Get
            Return _UserCreate
        End Get
        Set(value As String)
            _UserCreate = value
        End Set
    End Property

    Public Property DateCreate As Date
        Get
            Return _DateCreate
        End Get
        Set(value As Date)
            _DateCreate = value
        End Set
    End Property

    Public Property UserUpdate As String
        Get
            Return _UserUpdate
        End Get
        Set(value As String)
            _UserUpdate = value
        End Set
    End Property

    Public Property DateUpdate As Date
        Get
            Return _DateUpdate
        End Get
        Set(value As Date)
            _DateUpdate = value
        End Set
    End Property
End Class
