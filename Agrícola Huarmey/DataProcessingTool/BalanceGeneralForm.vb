...
Public Class BalanceGeneralForm
    ...
    Private Sub BalanceGeneralForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        dtPrint1 = dsMain.Tables("ReportLayout")
        dtPrint2 = dsMain.Tables("ReportLayout")
        Try
            LoadCompany()
    ...
    Private Sub DataProcess()
        ...
        sQuery = " SELECT am.id as move_id, am.name as move, l.id as line_id, aj.sequence_report as sequence_report, "
        sQuery += " aj.id as journal_id, aj.name as journal, am.date as date, l.name as glosa, l.ref as ref, "
        sQuery += " l.debit as debit, l.credit as credit, l.amount_currency as amount_currency, l.currency_id as currency_id, "
        sQuery += " ac.code as account_code, ac.name as account_name "
        ...
        If dtQuery.Columns.Count = 0 Then
            ...
            Return
        End If
        dtPrint = ExecuteAccessQuery("select * from ReportLayoutQry where report_id = 2")
        For Each row As DataRow In dtPrint.Rows
            ...
            End If
        Next
        dtPrint.Select("account_id='0'")(0)(9) = dAmount1 * -1
        dtPrint.Select("account_id='0'")(0)(10) = dAmount2 * -1
        ...
        For Each row As DataRow In dtQuery.Rows
            If Year(row("date")) = fiscal_year Then
                If Strings.Left(row("account_code"), Len(chart)) = chart Then
                    dResult += (row("debit") - row("credit")) * factor
                End If
            End If
        Next
        Return dResult
    ...
    Private Sub lueCompany_TextChanged(sender As Object, e As EventArgs) Handles seFiscalPeriod.TextChanged, seFiscalYear.TextChanged
        InitializeGrid()
    End Sub
